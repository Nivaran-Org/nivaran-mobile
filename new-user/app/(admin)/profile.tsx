...
export default function ProfileScreen() {
  ...
  const statCards = [
    { label: 'Total',       value: stats.total,      icon: <FileText  color="#1E3A8A" size={18} />, bg: '#DBEAFE', text: '#1E3A8A' },
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <View style={styles.avatarSection}>
          ...
          <Text style={styles.userName}>{user?.displayName ?? user?.name ?? 'Admin User'}</Text>
        ...
        <View style={styles.card}>
          <InfoRow icon={<User color="#1E3A8A" size={16} />} label="Full Name"  value={user?.name ?? '—'} />
          <Divider />
          <InfoRow icon={<Mail color="#1E3A8A" size={16} />} label="Email"     value={user?.email ?? '—'} />
          <Divider />
          <InfoRow icon={<Shield color="#1E3A8A" size={16} />} label="Role"    value={roleLabel} />
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.aboutHeader}>
            ...
            <View>
              <Text style={styles.aboutAppName}>NIVARAN ADMIN</Text>
              <Text style={styles.aboutVersion}>Version 4.2.0</Text>
            </View>
          </View>
          <Text style={styles.aboutDesc}>
            Admin Interface for the AI-powered civic grievance redressal system. Manage, route, and monitor
            grievances across all departments with real-time analytics.
          </Text>
        ...
        <TouchableOpacity style={styles.logoutBtn} onPress={handleLogout} activeOpacity={0.8}>
          ...
          <Text style={styles.logoutText}>Log Out from Command</Text>
        </TouchableOpacity>

        <Text style={styles.footer}>
          © 2026 NIVARAN · Admin Command Center{'\n'}Secure Access · Live Database
        </Text>
      </ScrollView>
    </SafeAreaView>
  );
